...
for word in "${wordlist[@]}"; do
    # Count the occurrences and output the matching lines
    matches=$(grep -r -w -i -n "${include_args[@]}" "${exclude_args[@]}" "$word" "$directory")
    count=$(echo "$matches" | wc -l)
    total_count=$((total_count + count-1))
    echo "$matches"  # Output the matching lines
done

echo "Search finished! Total lines found: $total_count"
